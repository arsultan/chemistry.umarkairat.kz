...
export const QuestsView: React.FC<QuestsViewProps> = ({
  ...
  const [subTab, setSubTab] = useState<'quiz' | 'quests'>('quiz');
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 py-3">
      ...
      {/* Top Sub-Navigation: Quiz Arena vs Lab Quests */}
      <div className="flex items-center justify-center">
        <div className="p-1.5 rounded-2xl bg-slate-100 dark:bg-white/[0.06] border border-slate-200 dark:border-white/[0.08] flex items-center gap-1.5 shadow-sm">
          <button
            onClick={() => {
              soundEffects.playAtomAdd();
              setSubTab('quiz');
            }}
            className={`px-5 py-2.5 rounded-xl text-xs sm:text-sm font-mono font-bold transition-all cursor-pointer flex items-center gap-2 ${
              subTab === 'quiz'
                ? 'bg-[#7c6ff6] text-white shadow-md shadow-[#7c6ff6]/25'
                : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
            }`}
          >
            <Flame className="w-4 h-4 text-amber-300" />
            <span>{language === 'kk' ? 'Олимпиадалық Квиз-Арена' : language === 'en' ? 'Olympiad Quiz Arena' : 'Олимпиадный Квиз-Турнир'}</span>
          </button>
          ...
          >
            ...
            <span>{language === 'kk' ? 'Зертханалық Квесттер & Жетістіктер' : language === 'en' ? 'Lab Missions & Achievements' : 'Лабораторные Квесты & Награды'}</span>
          </button>
        </div>
      </div>
      ...
      {/* 2. LAB QUESTS & ACHIEVEMENTS VIEW */}
      {subTab === 'quests' && (
          ...
          <div className="p-6 rounded-3xl bg-white dark:bg-slate-900/80 border border-slate-200/80 dark:border-white/[0.08] shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white tracking-tight">
                {language === 'kk' ? 'Мектеп зертханалық бағдарламасының квесттері' : language === 'en' ? 'School Laboratory Curriculum Quests' : 'Школьные лабораторные квесты'}
              </h1>
